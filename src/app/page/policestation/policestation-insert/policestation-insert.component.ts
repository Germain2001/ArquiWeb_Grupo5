...
export class PolicestationInsertComponent implements OnInit {
  ...
  aceptar() {
    if (this.policestation.NName.length > 0 && this.idAddressSeleccionado > 0) {
      let p = new Address();
      p.id = this.idAddressSeleccionado;
      this.policestation.address = p;
  ...
  init() {
    if (this.edicion) {
      this.policestationService.listarId(this.id).subscribe(data => {
        ...
        console.log(data);
        this.idAddressSeleccionado = data.address.id;
      });
